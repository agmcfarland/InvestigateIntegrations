...
test_that("extract_overlaps keeping non hits", {
  ...
  # Find 7 of 8 expected results. Fill in last hit with NA
  testthat::expect_equal(nrow(test_results), 8)

  # Find same query found twice in subject. Fill in last hit with NA
  two_separate_hits_same_query <- test_results %>%
    dplyr::filter(seqnames_query == 'chr3')

  testthat::expect_equal(nrow(two_separate_hits_same_query), 3)
  ...
  testthat::expect_equal(nrow(test_results), 2002)
